...
def hyperparam_search(mode=BINARY):
    ...
    for normalize in [True, False]:
        ...
        for C in [0.1, 1, 10, 100]:
            val_results.append(get_results(train_X, train_y, val_X, val_y, normalize=normalize, C=C))
    for val_result in val_results:
        print(val_result)

def get_results(train_X, train_y, val_X, val_y, normalize=False, C=1):
    print("Fitting Model...")
    log_reg = LogisticRegression(C=C, solver='lbfgs', multi_class='multinomial').fit(train_X, train_y)
